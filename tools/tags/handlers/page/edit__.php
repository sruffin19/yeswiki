...
if (!CACHER_MOTS_CLES && $this->hasAccess("write") && $this->hasAccess("read"))
{
    ...
    $pluginOutputNew = preg_replace ('/\<div class=\"form-actions\">/',
    $formtag.'<div class="form-actions">', $pluginOutputNew);
}

// If the page is an ebook, we will display the ebook generator
if ($this->hasAccess('write') && isset($pageeditionebook)) {
    $pluginOutputNew = preg_replace ('/(<div class="page">.*<hr class="hr_clear" \/>)/Uis',
    '<div class="page">'."\n".$pageeditionebook."\n".'<hr class="hr_clear" />', $pluginOutputNew);
}
